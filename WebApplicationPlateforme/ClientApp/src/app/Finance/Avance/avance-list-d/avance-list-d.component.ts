...
export class AvanceListDComponent implements OnInit {
  ...
  getDep() {
    this.avanceService.Get().subscribe(res => {
      this.GfactList = res;
      this.factList = this.GfactList.filter(item => item.attribut2 == "موافقة")
    })
  }
  ...
  populateForm(facture: Avance) {
    ...
    if (this.fact.attribut2 == "في الإنتظار" || this.fact.attribut2 == "غير معتمدة" || this.fact.attribut2 == "رفض") {
      this.etatok = false;
    } else
      this.etatok = true;
  ...
  accept() {
    this.fact.attribut2 = "معتمدة"
    this.fact.etatD = "معتمدة"
  ...
  refuse() {
    ...
    if (this.raisonRefus != null) {
      ...
      this.fact.attribut2 = "غير معتمدة"
      this.fact.etatD = "غير معتمدة"
      this.fact.dateD = this.date;
      this.fact.nomD = this.UserNameConnected;
      this.fact.idD = this.UserIdConnected;
